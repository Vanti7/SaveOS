...
- Langage(s) et versions : Python 3.11 (`api/`, `worker/`, `agent/` — FastAPI, SQLAlchemy, RQ, Borg) ; TypeScript / Next.js 14 App Router (`web/`) ; Bash/PowerShell (`scripts/`).
- Commandes :
  - install : `pip install -r requirements.txt` ; `cd web && npm install`
  - tests : `pytest tests/ -m "not integration"` (Python, fixtures SQLite via `tests/conftest.py` — jamais de DB/réseau réel hors tests marqués `integration`) ; `cd web && npm run test` (Vitest)
  - lint : `flake8 api/ worker/ agent/ --max-line-length=100 --ignore=E203,W503` + `black --check api/ worker/ agent/` (Python, informatif en CI, pas bloquant) ; `cd web && npm run lint` (ESLint, bloquant en CI)
  - run : `docker compose up -d --build` (stack complète : postgres, redis, minio, api, worker, web, prometheus, grafana en local)
- Fichier de version : `VERSION` (source de vérité unique, propagée à `web/package.json` et `setup.py` par `scripts/version.py bump --type ...`).
- Conventions spécifiques :
  - Séparation stricte api/worker/agent/web ; le worker exécute Borg côté serveur (l'agent ne l'exécute jamais localement) et isole chaque job dans un processus enfant forké par RQ.
  - Tests sous `tests/` (racine du dépôt), un fichier par module/fonctionnalité ; CI bloquante sur `pytest`/Vitest via `.github/workflows/simple-ci.yml` (voir `docs/CI-CD.md`).
  - Branches `feat/*`/`fix/*` depuis `develop`, PR vers `develop` (jamais de push direct) ; `master`/`main` réservé aux releases.
  - Décisions d'architecture notables consignées dans `docs/adr/`.
